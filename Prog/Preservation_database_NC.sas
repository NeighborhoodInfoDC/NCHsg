/**************************************************************************
 Program:  Preservation_database_NC.sas
 Library:  NCHsg
 Project:  North Carolina Affordable Housing Assessment
 Author:   W. Oliver
 Created:  01/11/19
 Version:  SAS 9.4
...
download for whole metro area or states if easier. We would like to be able to understand where properties are located, how many units are subsidized (at what level if known), subsidy programs involved, and any expiration dates for the subsidies.
 ...
 Modifications: 01/24/20 LH Adapted for ABQ from RegHsg project.
				01/29/20 LH Adapted for NC Housing project 
**************************************************************************/

%include "L:\SAS\Inc\StdLocal.sas";

** Define libraries **;
%DCData_lib( NCHsg, local=n )
 
data WORK.Test;
      %let _EFIERR_ = 0; /* set the ERROR detection macro variable */
      infile 'L:\Libraries\NCHsg\Raw\Active and Inconclusive Properties NC.csv' delimiter = ',' MISSOVER DSD lrecl=32767 firstobs=2;
...
where CountyCode=" ";
var NHPDPropertyID PropertyName PropertyAddress City State Zip PropertyStatus TotalUnits CountyCode CBSACode;
run;
